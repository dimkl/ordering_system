...
const OrderItemTransition = require('./orderItemTransition');
...
const FSM = StateMachine.factory({
  ...
  transitions: [
    ...
    { name: 'cancel', from: ['draft', 'placed', 'in_progress'], to: 'canceled' },
  ...
  methods: {
    ...
    onPlace: (lifecycle, order) => {
      return Promise.all(order.order_items.map(
        (oi) => OrderItemTransition.process(oi, order, 'place')
      ));
    }
  }
});
...
module.exports = OrderTransition;