...
describe("GET /time_slots", () => {
  ...
  it("returns all time_slots", async () => {
    ...
    const response = await request.get("/time_slots")
  ...
  it("returns specified time_slot", async () => {
    ...
    const response = await request.get("/time_slots/" + timeSlot.id)
      .set("Accept", "application/json");
  ...
  it("returns specified time_slot using uuid", async () => {
    ...
    const response = await request.get("/time_slots/" + timeSlot.uuid)
      .set("Accept", "application/json");
  ...
  it("throws 404 when specified time_slot does not exist", async () => {
    const response = await request.get("/time_slots/" + Math.floor((Math.random() * 100)))
      .set("Accept", "application/json");
